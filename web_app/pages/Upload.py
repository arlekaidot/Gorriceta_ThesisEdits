...
if choice == "Upload":
    ...
    file = st.file_uploader("Upload Your Dataset (Please Use CSV Format)")
    if file:
...
if choice == "Modelling":
    chosen_target = st.selectbox('Choose Your Dependent Variable', df.columns)
    x = df.drop([chosen_target],axis = 1).values
    chosen_size = st.slider('Select Test Size, (Normal Test Size is 0.2)', 0.0, 1.0)
